...
main <- function(args){
  ...
  dfs_agg[["daisy"]] <- dfs_agg[["daisy"]] %>% 
    mutate(Cohort=ifelse(grepl("-T1", Sample_Id), paste0(toupper(Cohort), "-T1"), paste0(toupper(Cohort),"-T2")))

  # load clinical tables
  dfs_cln <- setNames(lapply(args$clns, function(x) load_table(x)), args$cohorts)
  dfs_cln <- setNames(lapply(args$cohorts, function(x) dfs_cln[[x]] %>% mutate(Cohort=x)), args$cohorts)
  dfs_cln[["daisy"]] <- dfs_cln[["daisy"]] %>% rename(Tumor_Type=Project_TCGA_More) %>% 
    filter(WES_somatic_cna==1, WES_somatic_maf==1, Keep==1) %>% 
    mutate(Cohort=paste(toupper(Cohort), Biopsy_Number, sep="-"))

  # aggregate cohorts
  df_agg <- bind_rows(dfs_agg) %>% distinct()
  df_cln <- bind_rows(dfs_cln) %>% distinct()

  # table of tumor type counts
  df_tt_count <- df_cln %>% group_by(Tumor_Type, Cohort) %>% summarize(Size=n(), .groups="drop") %>%
    mutate(Cohort=toupper(Cohort))
  df_tt_count <- df_tt_count %>% spread(Cohort, Size)
...
if (getOption('run.main', default=TRUE)) {
  ...
  parser$add_argument("--clns", nargs="+", help="Paths to clinical tables")
