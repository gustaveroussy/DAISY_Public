...
select_samples <- function(df, df_cln){
  col_pair <- "Cluster_Id_DNA_P"
  df <- df %>% unite(!!col_pair, Tumor_Sample_Barcode, Matched_Norm_Sample_Barcode, sep="_vs_", remove=F)
  mask <- df[[col_pair]] %in% df_cln[[col_pair]]
  n_pair_bef <- length(unique(df[[col_pair]]))
  df <- df[mask,]
  n_pair_aft <- length(unique(df[[col_pair]]))
  df <- df %>% select(-all_of(col_pair))
  cat(paste("-INFO: selected data from", paste0(n_pair_aft,"/",n_pair_bef), "pairs present in the clinical table\n"))
  df
}
...
collapse_oncokb_annotations <- function(df_agg){
  ...
  df_agg <- df_agg %>% rename(Oncokb_Oncogenic=ONCOGENIC)
  ...
  cat("done!\n")
  cols <- c("Oncokb_Annotated", "Oncokb_Oncogenic", 
...
main <- function(args){
  ...
  df_mut <- select_samples(df_mut, df_cln)
  ...
  df_agg <- df_agg %>% rename(Cluster_Id=SAMPLE_ID)
  cols_ids <- c("Cluster_Id", "Sample_Id", "Subject_Id")
  df_agg <- left_join(df_agg, df_ids[,cols_ids] %>% distinct(), by="Cluster_Id")
  cols_cln <- c("Sample_Id_DNA_T", "Tumor_Type", "MSKCC_Oncotree", "Civic_Disease")
  df_agg <- left_join(df_agg, df_cln[,cols_cln] %>% distinct(), by=c("Sample_Id"="Sample_Id_DNA_T"))

  # column bind and joins
  cols_cln <- setdiff(cols_cln, "Sample_Id_DNA_T")
  cols_agg <- c("Variant_Classification", "t_vaf")
  df_fin <- bind_cols(df_agg[c("Subject_Id", "Sample_Id", "Cluster_Id", cols_cln, cols_agg, "Row_Id")], df_agg_alteration)
...
if (getOption('run.main', default=TRUE)) {
  ...
  parser$add_argument("--ids", type="character", help="Path to input curated clincal table.")
